...
LICECaptureCompressor *g_cap_pcf;
#endif
...
void UpdateCaption(HWND hwndDlg)
{
  ...
  {
#ifdef REAPER_LICECAP
    SetWindowText(hwndDlg,"REAPER_PPSCAP " LICECAP_VERSION " [stopped]");
#else
    SetWindowText(hwndDlg,"PPSCAP " PPSCAP_VERSION " [stopped]");
#endif
  }
  else
  {
    ...
    snprintf(buf,sizeof(buf),"%s%.100s - ppscap%s",pbuf,p,pbuf[0]?"":g_cap_state==1?" [recording]":" [paused]");
...
void WriteTextFrame(const char* str, int ms, bool isTitle, int w, int h, double bgAlpha=1.0f)
{
  ...
  {
    int del=0;
	g_cap_pcf->OnFrame(g_cap_bm, del);
    if (!isTitle)
    {
...
void SaveConfig(HWND hwndDlg)
{
  ...
  WritePrivateProfileString("PPSCAP","wnd_r",buf,g_ini_file.Get());
  sprintf(buf, "%d", g_max_fps);
  WritePrivateProfileString("PPSCAP","maxfps",buf,g_ini_file.Get());
  sprintf(buf, "%d", g_prefs);
  WritePrivateProfileString("PPSCAP","prefs",buf,g_ini_file.Get());
  sprintf(buf, "%d", g_titlems);
  WritePrivateProfileString("PPSCAP","titlems",buf,g_ini_file.Get());
  sprintf(buf, "%d", g_gif_loopcount);
  WritePrivateProfileString("PPSCAP","gifloopcnt",buf,g_ini_file.Get());
  sprintf(buf, "%d", g_stop_after_msec);
  WritePrivateProfileString("PPSCAP","stopafter",buf,g_ini_file.Get());
  
  WritePrivateProfileString("PPSCAP","title",g_title,g_ini_file.Get());

#ifdef VIDEO_ENCODER_SUPPORT
  sprintf(buf, "%d", g_cap_video_vbr);
  WritePrivateProfileString("PPSCAP","video_vbr",buf,g_ini_file.Get());
  sprintf(buf, "%d", g_cap_video_abr);
  WritePrivateProfileString("PPSCAP","video_abr",buf,g_ini_file.Get());
#endif

}

static WDL_DLGRET PpsCapMainProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg)
  {
    case WM_INITDIALOG:
      g_hwnd=hwndDlg;
#ifdef _WIN32
      SetClassLongPtr(hwndDlg,GCLP_HICON,(LPARAM)LoadIcon(GetModuleHandle(NULL),MAKEINTRESOURCE(IDI_ICON)));
      ...
      g_gif_loopcount = GetPrivateProfileInt("PPSCAP","gifloopcnt",g_gif_loopcount,g_ini_file.Get());
      g_max_fps = GetPrivateProfileInt("PPSCAP", "maxfps", g_max_fps, g_ini_file.Get());
      ...
      {
        char buf[1024];
        GetPrivateProfileString("PPSCAP","wnd_r","",buf,sizeof(buf),g_ini_file.Get());
        int a[4]={0,};
        const char *p=buf;
        int x;
        const int maxcnt  = 4;
      ...
      g_prefs = GetPrivateProfileInt("PPSCAP", "prefs", g_prefs, g_ini_file.Get());
      g_titlems = GetPrivateProfileInt("PPSCAP", "titlems", g_titlems, g_ini_file.Get());
      g_stop_after_msec = GetPrivateProfileInt("PPSCAP", "stopafter", g_stop_after_msec, g_ini_file.Get());

      GetPrivateProfileString("PPSCAP","title","",g_title,sizeof(g_title),g_ini_file.Get());

#ifdef VIDEO_ENCODER_SUPPORT
      g_cap_video_vbr = GetPrivateProfileInt("PPSCAP", "video_vbr", g_cap_video_vbr, g_ini_file.Get());
      g_cap_video_abr = GetPrivateProfileInt("PPSCAP", "video_abr", g_cap_video_abr, g_ini_file.Get());
#endif
      ...
      switch (LOWORD(wParam))
      {
        ...
        case IDC_REC:
          ...
          if (!g_cap_state)
          {
            ...
            {
              WritePrivateProfileString("PPSCAP","lastfn",g_last_fn,g_ini_file.Get());
...
#ifndef NO_LCF_SUPPORT
              if (strlen(g_last_fn)>4 && !stricmp(g_last_fn+strlen(g_last_fn)-4,".pcf"))
              {
                g_cap_pcf = new LICECaptureCompressor(g_last_fn,w,h);
                if (!g_cap_pcf->IsOpen())
                {
                  delete g_cap_pcf;
				  g_cap_pcf = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
  ...
  GetPrivateProfileString("PPSCAP","lastfn","",g_last_fn,sizeof(g_last_fn),g_ini_file.Get());

  g_hInst = hInstance;
  DialogBox(hInstance,MAKEINTRESOURCE(IDD_PPSCAP),GetDesktopWindow(),PpsCapMainProc);
  return 0;
}
    ...
    case SWELLAPP_ONLOAD:
      {
        ...
        *p=0;
        ...
        else
        {
          ...
          {
            g_ini_file.Set(p);
            g_ini_file.Append("/Library/Application Support/ppscap");
